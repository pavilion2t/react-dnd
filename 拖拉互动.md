...
This part assumes you are at least somewhat familiar with the concepts presented in the overview, such as the backends, the collecting functions, the types, the items, the drag sources, and the drop targets. 这部分假定你已经熟悉了概览里面的概念，比如后端，收集函数，类型，项目，拖放源和拖放目标。  
...
DragSource的高阶组件接收三个参数：type, spec, and collect。
...
For the Knight, the drag source specification is going to be ridiculously simple:  对于骑士，拖放源的spec就非常简单了： 
...
This time, we can't avoid passing the position to the Square. 这里 我们不能避免要把位置传给方块。 

After all, how can the Square know where to move the dragged knight if the Square doesn't know its own position? On the other hand, it still feels wrong because the Square as an entity in our application has not changed, and if it used to be simple, why complicate it? When you face this dilemma, it's time to separate the smart and dumb 哑的 components.

I'm going to introduce a new component called the BoardSquare. 这里新介绍一个组件BoardSquare。   

It renders the good old Square, but is also aware of its position. 它会渲染好方块，同时也知道它的位置。  
...
Let's now wrap the BoardSquare with a DropTarget.   现在我们用DropTarget封装BoardSquare。  
